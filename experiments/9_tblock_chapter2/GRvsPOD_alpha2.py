...
tb_1.setxi_train(N_GRxi_train, sampling=distribution)
...
tb_2.setxi_train(N_GRxi_train, sampling=distribution)
...
plt.plot(np.log10(mean_error_u[:,1]),'r',label='Standard Greedy')
plt.plot(np.log10(mean_error_u[:,2]),'b-o',label='Weighted Greedy')
plt.plot(np.log10(mean_error_u[:,3]),'y-*',label='Standard POD')
plt.plot(np.log10(mean_error_u[:,4]),'g--',label='Monte-Carlo POD')
plt.title('error comparison: second parameter')
plt.xlabel('$N$')
