...
class EblockPOD(EllipticCoercivePODBase):
    ...
    def assemble_truth_f(self):
        ...
        f = self.f
        # Define
        f0 = inner(f,v)*ds(5) + inner(l,v)*dx
        f1 = inner(f,v)*ds(6) + inner(l,v)*dx
        # Assemble
        F0 = assemble(f0)
        F1 = assemble(f1)
        # Return
...
eb_0.setxi_train(N_xi_train_POD, sampling=distribution)
...
eb_1.setxi_train(N_xi_train_POD, sampling=distribution)
...
eb_2.setxi_train(N_xi_train_POD, sampling=distribution)
...
eb_3.setxi_train(N_xi_train_RB, sampling=distribution)
...
eb_4.setxi_train(N_xi_train_RB, sampling=distribution)
