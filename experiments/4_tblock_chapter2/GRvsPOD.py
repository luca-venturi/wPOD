...
class TGRblock(EllipticCoerciveRBBase):
    ...
    def __init__(self, V, subd, bound):
        ...
        super(TGRblock, self).__init__(V, bc_list)
...
class TPODblock(EllipticCoercivePODBase):
    ...
    def __init__(self, V, subd, bound):
        ...
        super(TPODblock, self).__init__(V, bc_list)
        self.dx = Measure("dx")(subdomain_data=subd)
        self.ds = Measure("ds")(subdomain_data=bound)
        # Use the H^1 seminorm on V as norm, instead of the H^1 norm
        u = self.u
...
# 0.
Nmax = 10 
mean_error_u = np.zeros((Nmax,4))
mu_range = [(1.0, 3.0),(1.0, 3.0),(1.0, 3.0),(1.0, 3.0)]
first_mu = (2.0,2.0,2.0,2.0)
...
plt.plot(np.log10(mean_error_u[:,0]),'r',label='Standard Greedy')
plt.plot(np.log10(mean_error_u[:,1]),'b-o',label='Weighted Greedy')
plt.plot(np.log10(mean_error_u[:,2]),'y-*',label='Standard POD')
plt.plot(np.log10(mean_error_u[:,3]),'g--',label='Gauss-Jacobi POD')
...
m = min(mean_N_err) #
print [i for i in range(4) if mean_N_err[i] == m] #
for i in range(4): #
