...
plt.plot(np.log10(mean_error_u[:,2]),'c',label='WPOD - Linspace')
plt.plot(np.log10(mean_error_u[:,3]),'y',label='WPOD - ClenshawCurtis')
plt.plot(np.log10(mean_error_u[:,4]),'k',label='WPOD - GaussLegendre')
plt.plot(np.log10(mean_error_u[:,5]),'g',label='WPOD - GaussJacobi')
plt.legend()
plt.show()

# 8. Returns the best method #

mean_N_err = np.mean(np.log10(mean_error_u[:,1:6]), axis=0) #
m = min(mean_N_err) #
print [i for i in range(1,6) if mean_N_err[i-1] == m] #
for i in range(1,6): #
