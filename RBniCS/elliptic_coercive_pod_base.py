...
class EllipticCoercivePODBase(EllipticCoerciveBase):
    ...
    def set_density(self, weight=None):
        ...
        self.POD.set_density(xi_train_weight=weight.density(self.mu_range, self.xi_train))
        
    def set_weighted_flag(self, weighted_flag):
        self.POD.set_weighted_flag(weighted_flag)
    ...
    def error_analysis(self, N=None):
        ...
        for n in range(N): # n = 0, 1, ... N - 1
            mean_error = np.mean(error[n, :])
            print str(n+1) + " \t " + str(mean_error)
            mean_error_u[n] = mean_error
