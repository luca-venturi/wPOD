...
class EllipticCoerciveRBBase(EllipticCoerciveBase):
    ...
    def setxi_train(self, ntrain, enable_import=False, sampling=None):
        # Call Parent
        EllipticCoerciveBase.setxi_train(self, ntrain, enable_import, sampling)
    
    def set_density(self, weight=None):
        if weight == None: #
            original_density = UniformWeight() #
            weight = IndicatorWeight(original_density, -1e8) #
        self.xi_train_weight = weight.density(self.mu_range, self.xi_train)
    ...
    def offline(self):
        ...
        run = 0
        delta = 100.0
        while (run < self.Nmax and delta > self.tol):
            #print "############################## run = ", run, " ######################################"
            print run, " -> ", self.mu
    ...
    def greedy(self):
        ...
        for i in range(len(self.xi_train)):
            ...
            self._online_solve(self.N)
            delta = self.get_delta()
            if self.weighted_flag == 1:
                delta *= self.xi_train_weight[i]
            elif self.weighted_flag == 2:
                delta *= (self.xi_train_weight[i]**2)
            elif self.weighted_flag == 3:
                delta *= (self.xi_train_weight[i]**0.5)
    ...
    def error_analysis(self, N=None):
        ...
        print "N \t gmean(err_u)"
        mean_error_u = np.zeros((N))
        for n in range(N): # n = 0, 1, ... N - 1
            mean_error_u[n] = np.mean(error_u[n, :])
            ...
            print str(n+1) + " \t " + str(mean_error_u[n])
    ...
    def get_alpha_lb(self):
        ...
        print "Abort program."
        sys.exit("Please define function get_alpha_lb(self)!")
