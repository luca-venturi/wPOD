...
class ProperOrthogonalDecomposition():
    ...
    def set_density(self, xi_train_weight):
        xi_train_weight_vector = np.array(xi_train_weight)
        self.xi_train_weight = np.matrix(np.diag(xi_train_weight_vector))   
    ...
    def apply(self, S, eigenvalues_file, Nmax, tol):
        ...
        if self.weighted_flag == 1:
            corr = np.matrix(np.dot(self.xi_train_weight,corr))
        elif self.weighted_flag == 2:
            corr = np.matrix(np.dot(corr,self.xi_train_weight))
        elif self.weighted_flag == 3:
            corr = np.matrix(np.dot(np.matrix(np.dot(corr,self.xi_train_weight)),corr))
